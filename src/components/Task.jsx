...
function Task({task,delTodo,todoID,updTodo,isComplete}) {
  return (
    <div className='bg-white rounded-lg flex p-4 my-2 items-center h-full '>
        {
            isComplete ? <FaCheckCircle color='green' className='mr-3'/> : <span className='w-5 h-5 p-1 bg-orange-400 rounded-full mr-3'></span>
        }
        
        <p className='grow text-gray-500 font-semibold'>{task.task}</p>
        <div className='flex items-center justify-between gap-4'>
            <span className='font-medium'>{task.time ? task.time : '00:00'}</span>
